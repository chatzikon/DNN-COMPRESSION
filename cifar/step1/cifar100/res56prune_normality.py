...
parser.add_argument('--model', default='./pretrained_models/resnet56_c100_0_7085.pth.tar', type=str, metavar='PATH',
...
for m in model.modules():
    if isinstance(m, nn.Conv2d):
        ...
        if layer_id % 2 == 0:
            ...
            W, p = stats.shapiro(a)
            prune_prob_stage = np.ceil(10 * (W - 0.75) / (factor * 0.2)) / 10
...
torch.save({'cfg': cfg, 'state_dict': newmodel.state_dict()}, os.path.join(args.save, 'res56_c100.pth.tar'))
...
with open(os.path.join(args.save, "res56_c100.txt"), "w") as fp:
