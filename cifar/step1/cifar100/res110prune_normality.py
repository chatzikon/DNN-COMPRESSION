...
parser.add_argument('--model', default='./pretrained_models/resnet110_c100_0.7241.pth.tar', type=str, metavar='PATH',
    ...
    if isinstance(m, nn.Conv2d):
        ...
        if layer_id % 2 == 0:
            ...
            if args.metric == 'l1norm':
                ...
                filter_values = np.zeros(weight_copy.shape[0])
                for i in range(weight_copy.shape[0]):
                    filter_values[i] = stats.kstat(weight_copy[i, :, :, :], 3) * stats.kstat(
                        weight_copy[i, :, :, :], 4)
...
torch.save({'cfg': cfg, 'state_dict': newmodel.state_dict()}, os.path.join(args.save, 'res110_c100.pth.tar'))
...
with open(os.path.join(args.save, "res110_c100.txt"), "w") as fp:
