...
parser.add_argument('--model', default='./pretrained_models/mobilenetv2_0.934.pth.tar', type=str, metavar='PATH',
...
for m in model.modules():
    if isinstance(m, nn.Conv2d):
        ...
        if m.kernel_size == (1,1) and count==0:
            ...
            count=0
        elif m.kernel_size == (1,1) and count==1 and out_channels==temp:
            count=0
            continue
        ...
        if layer_id % 3 == 1 or layer_id % 3 == 2:
            ...
            W, p = stats.shapiro(a)
            prune_prob_stage = np.ceil(10 * (W - 0.3) / (factor * 0.7)) / 10
...
newmodel=models.mobilenetv2.MobileNetV2(num_classes=10,cfg=cfg)
if args.cuda:
    newmodel.cuda()
...
for [m0, m1] in zip(model.modules(), newmodel.modules()):
    if isinstance(m0, nn.Conv2d):
        if m0.kernel_size == (3, 3) and m0.in_channels == 3:
            ...
            count=1
            temp=m0.out_channels
        elif m0.kernel_size == (3,3):
            count=0
        elif m0.kernel_size == (1,1) and count==1 and m0.out_channels==temp:
            count=0
            m1.weight.data = m0.weight.data.clone()
            continue
...
print(cfg)
torch.save({'cfg': cfg, 'state_dict': newmodel.state_dict()}, os.path.join(args.save, 'mobilenetv2.pth.tar'))
...
with open(os.path.join(args.save, "mobilenetv2.txt"), "w") as fp:
