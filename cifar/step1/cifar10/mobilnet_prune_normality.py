...
parser.add_argument('--model', default='./pretrained_models/mobilenet_0.936.pth.tar', type=str, metavar='PATH',
...
model=models.mobilenet.MobileNet(num_classes=10)
...
for m in model.modules():
    ...
    if isinstance(m, nn.Conv2d):
        ...
        W, p = stats.shapiro(a)
        prune_prob_stage = np.ceil(10 * (W - 0.55) / (factor * 0.45)) / 10
...
newmodel = models.mobilenet.MobileNet(num_classes=10,cfg=cfg)
...
for [m0, m1] in zip(model.modules(), newmodel.modules()):
    ...
    if isinstance(m0, nn.Conv2d):
        ...
        if m0.kernel_size == (3, 3):
            mask_out = cfg_mask[layer_id_in_cfg]
            #mask_out = cfg_mask[layer_id_in_cfg-1]
            idx1 = np.squeeze(np.argwhere(np.asarray(mask_out.cpu().numpy())))
            if idx1.size == 1:
                idx1 = np.resize(idx1, (1,))
...
print('CFG')
print(cfg)
torch.save({'cfg': cfg, 'state_dict': newmodel.state_dict()}, os.path.join(args.save, 'mobilenet.pth.tar'))
...
with open(os.path.join(args.save, "mobilenet.txt"), "w") as fp:
