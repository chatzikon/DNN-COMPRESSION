...
class EarlyStopping:
    ...
    def __call__(self, val_loss, model):
        ...
        if self.best_score is None:
            ...
            self.counter += 1
            print('EarlyStopping counter:{:f} out of {:f} '.format(self.counter, self.patience))
...
def fine_tune(model, train_loader,valid_loader, device='cuda', save_dir=None,
    ...
    for epoch in range(start_epoch, ft_epochs):
        ...
        if kd_params is not None:
            train(train_loader, model, teacher_model, optimizer, epoch, kd_params, device, batches_per_train,
                  add_l1_penalty=add_l1_penalty, suffix=suffix, loggs_dir=loggs_dir)
        else:
            train(train_loader, model, None, optimizer, epoch, kd_params, device, batches_per_train,
                  add_l1_penalty=add_l1_penalty, suffix=suffix, loggs_dir=loggs_dir)
        ...
        print('VALLL')
        prec1, _, val_loss = validate(valid_loader, model, device, batches_per_val, suffix=suffix, loggs_dir=loggs_dir)
        #scheduler.step(val_loss)
        # remember best prec@1 and save checkpoint
        print(best_prec1)
        is_best = prec1.cpu().numpy() > best_prec1
        best_prec1 = max(prec1.cpu().numpy(), best_prec1)
        print(best_prec1)
        save_checkpoint(model, is_best, best_prec1, save_dir, suffix)
